 ...
 * Crée user (via Better Auth) + organisation liée
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!email || !password || !orgName) return res.status(400).json({ error: 'Champs requis manquants' })

    // 1. Créer user via Better Auth
    const auth = getAuth()
    const signUpRes = await auth.api.signUpEmail({
      body: { email, password, name: email.split('@')[0] },
      headers: req.headers,
    })
    if (signUpRes.error) return res.status(400).json({ error: signUpRes.error.message })
    ...
    // 3. Lier user → org (Better Auth crée la table users, on ajoute organization_id)
