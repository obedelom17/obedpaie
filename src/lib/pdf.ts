...
export async function generateBulletinPDF(data: BulletinData): Promise<jsPDF> {
  ...
  if (!logoLoaded) {
    doc.setFontSize(11); doc.setFont('times', 'bold'); doc.setTextColor(...DARK_BLUE)
  ...
  doc.setFont('times', 'bold'); doc.setFontSize(13); doc.setTextColor(192, 0, 0)
  ...
  doc.setFont('times', 'bold'); doc.setFontSize(11); doc.setTextColor(...BLACK)
  ...
  doc.setFont('times', 'bold'); doc.setFontSize(8.5); doc.setTextColor(...BLACK)
  const empName = `${employee.last_name} ${employee.first_name}`
  const infoLeft = ml + 4; let iy = y + 5.5
  const line = (label: string, val: string) => {
    doc.setFont('times', 'bold'); doc.text(label, infoLeft + 28, iy, { align: 'right' })
    doc.setFont('times', 'normal'); doc.text(val, infoLeft + 30, iy)
    iy += 4.2
  }
  ...
  doc.setFont('times', 'normal'); doc.text(`Date d'embauche:  ${hireDate}`, infoLeft, iy)
  iy += 4.5
  doc.text(`Pers à charge `, infoLeft, iy)
  doc.setFont('times', 'bold'); doc.text(String(employee.children_count || 0), infoLeft + 24, iy)
  y += 38
  ...
  doc.setFont('times', 'bold'); doc.setFontSize(9); doc.setTextColor(...RED_LINE)
  ...
  for (const row of retRows) {
    doc.rect(ml, y, mr - ml, rowH, 'D')
    doc.setFont('times', 'bold'); doc.text(row.label, COL.rub + 2, y + 3.5)
    doc.setFont('times', 'normal')
...
export function generateAttestationTravailPDF(employee: any, orgName: string) {
  ...
  doc.setTextColor(255, 255, 255); doc.setFontSize(13); doc.setFont('times', 'bold')
  doc.text('ATTESTATION DE TRAVAIL', pw / 2, 12, { align: 'center' })
  doc.setTextColor(0, 0, 0); doc.setFontSize(10); doc.setFont('times', 'normal')
  let y = 35
...
export function generateBordereauCNSS(period: any, variables: any[], orgName: string) {
  ...
  const monthName = MONTH_NAMES[period.period_month - 1]
  doc.setFillColor(37, 99, 235); doc.rect(0, 0, pw, 18, 'F')
  doc.setTextColor(255, 255, 255); doc.setFontSize(12); doc.setFont('times', 'bold')
  doc.text('BORDEREAU DE DÉCLARATION CNSS', pw / 2, 12, { align: 'center' })
  doc.setTextColor(0, 0, 0); doc.setFontSize(9); doc.setFont('times', 'normal')
